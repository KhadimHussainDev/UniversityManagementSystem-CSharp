...
namespace uams.DL
{
    class DegreeProgramDL
    {
        ...
        public static void storeintoFile(string path, DegreeProgram d)
        {
            ...
            string SubjectNames = "";
            for(int x = 0; x < d.getSubjects().Count - 1; x++)
            {
                SubjectNames = SubjectNames + d.getSubjects()[x].getType() + ";";
            }
            SubjectNames = SubjectNames + d.getSubjects()[d.getSubjects().Count - 1].getType();
            f.WriteLine(d.getDegreeName() + "," + d.getDegreeDuration() + "," + d.getSeats() + "," + SubjectNames);
            f.Flush();
            f.Close();
        }
